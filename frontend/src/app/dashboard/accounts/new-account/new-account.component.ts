  ...
  template: `
    <div class="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div class="grid gap-8 md:grid-cols-2">
        ...
        <div 
          (click)="selectAccount('COURANT')"
          [ngClass]="{
            'border-blue-600 ring-2 ring-blue-600/10 shadow-xl shadow-blue-900/5 -translate-y-2': accountData.accountType === 'COURANT',
            'border-slate-100 bg-white hover:border-blue-200 hover:shadow-lg': accountData.accountType !== 'COURANT'
          }"
        ...
        >
          <div 
            ...
            [ngClass]="accountData.accountType === 'COURANT' ? 'bg-blue-600 border-blue-600' : 'border-slate-200'"
          >
            <svg *ngIf="accountData.accountType === 'COURANT'" class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
            </svg>
          </div>

          <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-100 text-blue-600 mb-6 group-hover:scale-110 transition-transform">
        ...
        <div 
          (click)="selectAccount('EPARGNE')"
          [ngClass]="{
            'border-blue-600 ring-2 ring-blue-600/10 shadow-xl shadow-blue-900/5 -translate-y-2': accountData.accountType === 'EPARGNE',
            'border-slate-100 bg-white hover:border-blue-200 hover:shadow-lg': accountData.accountType !== 'EPARGNE'
          }"
          class="group relative cursor-pointer rounded-3xl border-2 p-8 transition-all duration-300"
        >
          <div 
            class="absolute top-6 right-6 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors"
            [ngClass]="accountData.accountType === 'EPARGNE' ? 'bg-blue-600 border-blue-600' : 'border-slate-200'"
          >
            <svg *ngIf="accountData.accountType === 'EPARGNE'" class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
            </svg>
          </div>
      ...
      <div class="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl overflow-hidden relative group">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <button
            ...
            [disabled]="loading"
            class="whitespace-nowrap px-10 py-5 bg-white text-slate-900 font-black rounded-2xl hover:bg-blue-50 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 shadow-xl"
          >
...
export class NewAccountComponent {
  ...
  loading = false;
  accountData = {
    accountType: 'COURANT' as 'COURANT' | 'EPARGNE'
  };

  selectAccount(type: 'COURANT' | 'EPARGNE'): void {
